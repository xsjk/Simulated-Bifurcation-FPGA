...
module dual_lutram_gen #(
    ...
    parameter ENABLE_OUTREG_A = 0,
    parameter ENABLE_OUTREG_B = 0
) (
...
generate
    for (i = 0; i < WIDTH; i = i + 1) begin : gen_bram_units
        ...
        if (ENABLE_OUTREG_A) always @(posedge clk) douta[i] <= douta_i;
        else                 always @(*) douta[i] = douta_i;
        if (ENABLE_OUTREG_B) always @(posedge clk) doutb[i] <= doutb_i;
