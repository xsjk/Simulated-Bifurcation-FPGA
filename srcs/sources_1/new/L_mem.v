...
generate
    for (gi = 0; gi < BLOCK_SIZE; gi = gi + 1) begin : gen_next_tmp_j
        ...
        if (ENABLE_STOREREG)
            always @(posedge clk) begin
                is_first_col <= stage_j[STAGE_ARRIVE-1] == 0;
                is_diagonal <= stage_i[STAGE_ARRIVE-1] == stage_j[STAGE_ARRIVE-1];
            end
        else
            always @(*) begin
                is_first_col = stage_j[STAGE_ARRIVE] == 0;
                is_diagonal = stage_i[STAGE_ARRIVE] == stage_j[STAGE_ARRIVE];
            end
